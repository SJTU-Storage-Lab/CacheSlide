"""Selective, request-local CacheSlide execution shared by CPU and native V1.
...
class CacheSlideRuntime:
    ...
    reference tests. Native V1 supplies real block-table-backed page arenas.
    """
    ...
    def run(
        ...
        if n > self.settings.max_prompt_tokens:
        ...
        prefill = step.positions == tuple(range(n))
        if prefill:
            ...
            reuse = plan.operation == "reuse" and hit
            state = RequestState(plan, reuse, profiles, reason)
            state.write_cache = (
                plan.operation == "populate" and not hit and bool(profiles)
            )
            state.metrics = {
