"""Opt-in native vLLM V1 model; importing this module requires the pinned engine.
...
class CacheSlideModel(LlamaModel):
    def __init__(self, *, vllm_config, prefix="", layer_type=None):
        ...
        for name in (
            ...
            if getattr(hf, name, None) != bundle.config[name]:
    ...
    def _arena(self, layer_index, prompt_length, max_selected, *, existing=None):
        ...
        if slot_mapping is None or len(slot_mapping) < prompt_length:
            raise ValueError("native slot mapping does not cover this prompt")
        expected = torch.tensor(
            [arena.canonical_slot(i) for i in range(prompt_length)],
            device=slot_mapping.device,
            dtype=slot_mapping.dtype,
        )
        if not torch.equal(slot_mapping[:prompt_length], expected):
