...
verification and original immutable receipts too.
...
```bash
python -m cacheslide_sglang.reproduction_report \
...
`status: complete`, `input_digest` (the exact token-array canonical SHA256), plus
the same phase/case/repeat/output/timing fields. Do not relabel CacheSlide's
