...
python -m cacheslide_core.training_pipeline prepare \
...
python -m cacheslide_core.training_pipeline train \
...
checkpoint directory or just the safetensors file. SGLang's production
`CacheSlideLlamaForCausalLM._init_model` verifies `AdapterBundle` against the
backbone before model allocation, loads each layer on the native parameter
device/dtype, and binds both LoRA deltas and CoPE to the attention wrapper.
The retained vLLM module follows the same verification/layer-loading contract.

For example, after creating CCPE profiles that bind this exact trained adapter
and the prepared layouts, a native SGLang run mounts it as follows:

```bash
python -m cacheslide_sglang.cli bench \
  --model /workspace/CacheSlide-paper-assets/models/Mistral-7B-Instruct-v0.2 \
  --adapter runs/hotpotqa-pilot-training/adapter \
  --profiles runs/hotpotqa-profiles \
  --input runs/hotpotqa-pilot-data/evaluation.inputs.jsonl \
  --seed-input runs/hotpotqa-pilot-data/population.inputs.jsonl \
  --cache-root runs/hotpotqa-native-cache \
  --output runs/hotpotqa-native-evaluation \
  --max-tokens 128 --dtype bfloat16 --run
```

Here `runs/hotpotqa-profiles` must already be a real calibrated profile directory;
it is not produced by training alone. Wrong-backbone adapters, incomplete
artifacts and mismatched profiles are rejected. `--adapter` skips automatic
retraining; it does not bypass source-attestation or supported-engine gates.
...
vLLM and SGLang CacheSlide model constructors and `CacheSlideRuntime`, and checks
logit agreement with the trained reference. Disabling the mounted LoRA or CoPE
changes outputs; a changed backbone is rejected before native allocation.
Only unavailable engine-native allocation/dispatch shells are replaced by tiny
frozen checkpoint projections in this test. It is **not** a native engine launch,
