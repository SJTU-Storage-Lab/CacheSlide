...
from cacheslide_vllm.reproduction_metrics import score_answer
from cacheslide_vllm.reproduction_report import (
    build_report,
    main,
    read_jsonl,
    validate_evaluation,
...
def fixture():
    ...
    for phase in ("warmup", "measured"):
        for operation in ("recompute", "reuse"):
            ids = [3, 4] if operation == "recompute" else [3, 5]
            request_id = phase + "-" + operation
            records.append(
                {
...
def report(records=None, manifest=None, **kwargs):
    ...
    return build_report(
        ...
        cases,
        lambda ids: " ".join(
            {3: "red", 4: "fox"}[token] for token in ids if token != 5
        ),
...
def test_invalid_measurement_cannot_be_dropped_to_claim_speedup(invalid):
    *_, records = fixture()
    row = records[-1]
    if invalid == "fallback":
        row["runtime_metrics"]["fallback"] = True
    elif invalid == "miss":
        row["runtime_metrics"]["cache_hit"] = False
    elif invalid == "digest":
        row["prompt_token_ids"] = [999, 999]
...
@pytest.mark.parametrize(
    "invalid", ["request", "operation", "error", "length", "count"]
)
def test_vllm_real_schema_invariants_are_checked(invalid):
    *_, records = fixture()
    row = records[-1]
    if invalid == "request":
        row["runtime_metrics"]["request_id"] = "different-request"
    elif invalid == "operation":
        row["runtime_metrics"]["operation"] = "recompute"
    elif invalid == "error":
        row["metrics_error"] = "worker receipt was unavailable"
    elif invalid == "length":
...
def test_vllm_report_does_not_accept_fabricated_sglang_evidence():
    *_, records = fixture()
    row = records[-1]
    row["output_ids"] = row.pop("generated_token_ids")
    row["receipt_valid"] = True
    row["receipt"] = {"status": "complete", "resources_released": True}
    _, summary = report(records=records)
    assert summary["status"] == "invalid_experiment"
    assert summary["worker_output_digest_and_release_attested"] is False


def test_report_accepts_actual_cli_benchmark_record_shape_without_native_engine():
    metadata = {
        "version": 1,
        "operation": "reuse",
        "namespace": "test",
        "task_id": "qa",
        "chunks": [
            {"id": "control", "role": "recompute", "start": 0, "end": 1},
            {"id": "fixed", "role": "reuse", "start": 1, "end": 2},
        ],
    }
    plan = RequestPlan.parse(metadata, [11, 12])
    case = cli.InputCase("held-out-case", (11, 12), plan)

    class SchemaOnlyEngine:
        def generate(self, prompt, *, sampling_params, use_tqdm):
            operation = sampling_params.extra_args["cacheslide"]["operation"]
            self.request_id = getattr(self, "request_id", "r") + "r"
            self.metrics = {
            ...
            return [
                SimpleNamespace(
                    request_id=self.request_id,
                    outputs=[SimpleNamespace(token_ids=[3, 4], finish_reason="length")],
                )
            ]
    ...
    records, _ = cli._benchmark_runs(
        SchemaOnlyEngine(),
        SimpleNamespace,
        SimpleNamespace(max_tokens=2, warmup=1, repeats=1, token_f1=False),
        [case],
        {"one-layout": case},
    )
    _, summary = report(records=records)
    assert summary["status"] == "valid_partial_experiment"
    assert summary["input_record_schema"] == "cacheslide_vllm.cli.generate_one"
    assert summary["worker_output_digest_and_release_attested"] is False
