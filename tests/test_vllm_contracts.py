...
def test_engine_supports_both_audited_runners_without_mutating_selection(
    monkeypatch, use_v2
):
    config = _config()
...
def test_unknown_native_runner_selection_is_rejected(value):
    config = _config()
    config.use_v2_model_runner = value
    with pytest.raises(CompatibilityError, match="runner selection"):
        validate_engine_config(config)


def _trained_model_config():
    return dict(
        model_type="llama",
        hidden_size=8,
...
def test_native_embedding_tying_must_match_the_trained_backbone(tied):
    trained = _trained_model_config() | {"tie_word_embeddings": tied}
    native = SimpleNamespace(**trained)
    validate_native_model_config(native, trained)
    native.tie_word_embeddings = not tied
    with pytest.raises(ValueError, match="tie_word_embeddings"):
        validate_native_model_config(native, trained)


def test_native_embedding_tying_defaults_to_false():
    trained = _trained_model_config()
    native = SimpleNamespace(**trained)
    validate_native_model_config(native, trained)
    native.tie_word_embeddings = False
    validate_native_model_config(native, trained)
    native.tie_word_embeddings = True
    with pytest.raises(ValueError, match="tie_word_embeddings"):
        validate_native_model_config(native, trained)


@pytest.mark.parametrize("side", ["native", "trained"])
@pytest.mark.parametrize("invalid", [0, 1, None, "false"])
def test_embedding_tying_requires_actual_booleans(side, invalid):
    trained = _trained_model_config() | {"tie_word_embeddings": False}
    native = SimpleNamespace(**trained)
    if side == "native":
        native.tie_word_embeddings = invalid
    else:
        trained["tie_word_embeddings"] = invalid
    with pytest.raises(ValueError, match="tie_word_embeddings"):
        validate_native_model_config(native, trained)
...
def test_native_overrides_cannot_enable_untrained_structure(field, value):
    trained = _trained_model_config()
    native = SimpleNamespace(**(trained | {field: value}))
...
def test_v1_preemption_replay_checks_actual_ids_and_preserves_original_prompt():
    runner = _Runner()
    runner.requests["req"].output_token_ids = [6, 7]
    attach_runner(runner, context_getter=lambda: _context(4))
    runner._model_forward(
        input_ids=torch.tensor([4, 5, 6, 7]), positions=torch.arange(4)
    )
    assert runner.observed[-1].prompt_token_ids == (4, 5)
    assert runner.observed[-1].replay_token_ids == (4, 5, 6, 7)
    with pytest.raises(ValueError, match="input IDs disagree"):
        runner._model_forward(
            input_ids=torch.tensor([4, 5, 6, 8]), positions=torch.arange(4)
        )
    assert current_step() is None
