...
def test_native_configuration_is_eager_single_request_and_no_gpu_by_default(
    ...
    assert result["model_runner"] == "v2"
    assert cli.main([*args, "--model-runner", "v1"]) == 0
    assert json.loads(capsys.readouterr().out)["model_runner"] == "v1"
...
def test_benchmark_pairs_lengths_excludes_warmup_and_saves_raw_outputs(
    ...
    assert summary["model_runner"] == "v2"
