...
from cacheslide_core.inputs import _reject_constant, _unique_object
...
def validate_record(row, case, variant, maximum):
    ...
    if not token_ids(ids) or len(ids) > maximum:
        errors.append("invalid_or_oversized_output_ids")
    if row.get("prompt_tokens") != len(case["prompt_token_ids"]):
        errors.append("prompt_length_mismatch")
    if not _positive_finite(row.get("elapsed_seconds")):
        errors.append("invalid_generation_duration")
    if variant == "native_baseline":
        if row.get("input_digest") != digest(case["prompt_token_ids"]):
            errors.append("native_input_digest_mismatch")
        if row.get("status") != "complete" or row.get("variant") != variant:
            errors.append("native_baseline_not_explicitly_complete")
    else:
        receipt = row.get("receipt") or {}
        if not isinstance(receipt, dict):
            return errors + ["invalid_native_cache_receipt"]
        ...
        if not (
            ...
            and receipt.get("input_digest") == digest(case["prompt_token_ids"])
            ...
            errors.append("invalid_native_cache_receipt")
