...
def validate_llama_config(config: dict) -> dict:
    ...
    if not valid_epsilon:
        raise ValueError("rms_norm_eps must be finite and positive")
    logit_scale = config.get("logit_scale", 1.0)
    if type(logit_scale) not in {int, float}:
        raise ValueError("logit_scale must be a finite number")
    try:
        valid_logit_scale = math.isfinite(logit_scale)
    except OverflowError:
        valid_logit_scale = False
    if not valid_logit_scale:
        raise ValueError("logit_scale must be a finite number")
